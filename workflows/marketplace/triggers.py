...
def select_scenarios(
    ...
    for event in activation_events:
        ...
        if event_type == "onView" and event_value:
            ...
            if str(event_value) in _BUILTIN_VIEW_IDS:
                register_attempt(
                    event_type=event_type,
                    event_value=event_value,
                    track=_HEURISTIC_TRACK,
                ...
                for index, scenario_name in enumerate(
                    HEURISTIC_EVENT_TYPE_TO_SCENARIOS.get(key, [])
                ):
                    mark_scenario(
                        scenario_name,
                        reason=(
                            f"built-in view trigger {key} treated as heuristic coverage"
                        ),
                        score=1_000 - index,
                        track=_HEURISTIC_TRACK,
                    )
            elif str(event_value) in contributed_view_ids:
                register_attempt(
                    event_type=event_type,
                    event_value=event_value,
                    track=_OFFICIAL_TRACK,
                ...
                if "webview" in str(event_value).lower():
                    mark_scenario(
                        "webview_probe",
                ...
                mark_scenario(
                    "project_exploration",
                    reason=(
                        f"unmapped view trigger {key} fell back to heuristic explorer "
                        "coverage"
                    ),
                    score=250,
                    track=_HEURISTIC_TRACK,
        ...
        if event_type in OFFICIAL_EVENT_REGISTRY:
            ...
            for index, scenario_name in enumerate(
                EVENT_TYPE_TO_SCENARIOS.get(event_type, [])
            ):
                mark_scenario(
                    scenario_name,
                    reason=f"activation {event_label}",
            ...
            for index, scenario_name in enumerate(
                HEURISTIC_EVENT_TYPE_TO_SCENARIOS[event_type]
            ):
                mark_scenario(
                    scenario_name,
                    reason=f"activation {event_label}",
        ...
        if event_type == "onUri" and publisher_name:
            ...
            official_extra_capabilities.add("uri_walkthrough")
        if event_type == "onFileSystem":
            official_extra_capabilities.add("workspace_fs")
        if event_type == "onEditSession":
            official_extra_capabilities.add("workspace_fs")
    ...
    if contributes_task_definitions or contributes_terminal_profiles:
        official_extra_capabilities.add("terminal_tasks")
    if capability_metadata and str(
        capability_metadata.get("untrusted_supported", "")
    ).lower() not in {
        "",
        "false",
        "unsupported",
