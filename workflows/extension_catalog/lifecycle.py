"""Extension catalog lifecycle: search, create, delete, and getters.

Owns the public extension-catalog surface called by the router and the
marketplace workflow: filesystem-driven create flows, search/delete by
identity, and the read-side getters. Manifest-to-ORM hydration is
delegated to `manifest_to_schema` so this module stays focused on the
