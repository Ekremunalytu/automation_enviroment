"""Filesystem scanner — locates and parses VS Code extension package.json files."""
...
def parse_json_file(json_path: Path) -> dict[str, Any] | None:
    """Parse a JSON file and return its contents, or None on error."""
    try:
...
def search_extension(extension_name_field: str) -> dict[str, Any] | None:
    ...
    for extension_dir in (p for p in extension_path.iterdir() if p.is_dir()):
