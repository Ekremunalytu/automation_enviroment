"""Event-attempt verification + coverage track reconciliation (W16-4 slimmed core).

W16-4 (2026-05-18) extracted the W13-1 security primitives and the
W13-12 handshake dispatch into sibling modules:

* ``security.py`` — ``HARNESS_PYTHON_SECRET_PATH``,
  ``load_harness_python_secret``, ``_verify_harness_marker_signature``.
* ``handshake.py`` — ``_harness_trace_records_by_attempt``,
  ``_attempt_has_harness_completion_trace``.

This module retains the event-attempt verification state machine
(``reconcile_event_attempts`` + status-mutation helpers + activation
matchers) and the coverage track reconciler (``reconcile_coverage_verification``
+ ``_reconcile_track``). Behavior is byte-identical with the pre-W16-4
682-LoC monolith; the W13-1 / W13-12 architecture gates were re-targeted
at the new sibling paths in the same pull. Test seam:
``tests/executor/test_playwright_health_reconciliation.py`` (state-
machine + W13-1 forgery/signature pins),
``tests/security/test_harness_handshake_required.py`` (W13-12 fail-
closed dispatch via the public ``reconcile_event_attempts`` facade).
"""
