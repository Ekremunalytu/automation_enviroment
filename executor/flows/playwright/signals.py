...
def _indexed_target_activations(report: Any) -> list[tuple[str, Any]]:
...
def _indexed_target_file_events(report: Any) -> list[tuple[str, Any]]:
    return [
...
def _indexed_target_network_events(report: Any) -> list[tuple[str, Any]]:
    return [
...
def _indexed_ui_blockers(report: Any) -> list[tuple[str, Any]]:
    return [
        (f"ui-blocker-{index:04d}", entry)
        for index, entry in enumerate(
            getattr(report, "ui_blocker_entries", []), start=1
        )
    ]
