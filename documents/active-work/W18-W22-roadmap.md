...
| W21-3 | `workspace_trust` her iki track → covered | `[GOAL taxonomy-workspace-trust-coverage]` (new) | **closed `2026-05-27`** via primary `c744c15` + self-stamp on `week21` branch. Taxonomy promotion + harness `isTrusted` baseline marker + `onDidGrantWorkspaceTrust` listener (OutputChannel route, W19-X paterni) + `workspace_trust_transition` scenario + 4 invariant tests + fixture regen. Runtime stimulus pass (untrusted → granted exercise) deferred to W22 as `[FOLLOWUP workspace-trust-stimulus-pass]` (workspace trusted-by-default in fixture; granted-transition exercise needs fixture restructuring). Live anchor `6fd7b959bd5a` confirms `missing_capabilities` 4 → 3 items, workspace_trust matrix entry status `covered` is_active=true with `workspace_trust_transition` advertised. |
