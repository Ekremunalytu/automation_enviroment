...
| W19-4 | Hat-2 | `onDebug*` event'leri nonce confirmation + consumer wire | `[FOLLOWUP harness-verification-debug-events]` (new) | onDebug + alt aileleri için confirmation üretimi; **+ consumer wire** `executor/flows/playwright/health/reconciliation.py` (`confirmation_source != "none"` ise `failure_reason_code="harness_verification_unconfirmed"` set edilmez) — yoksa W19 must-pass #2 düşmez |
