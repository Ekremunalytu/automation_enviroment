...
## Static Analysis Pre-Check Lane (deferred to next iter)

Static-analysis pre-check stream (formerly `extrace-static` parallel
branch) is **deferred to a future iter** — operator will resume it
serially off `main` after W22 closes. The four locked design decisions
(block-and-warn semantics, separate `automation_static_analyzer`
Docker service, schema-first contract landing, in-house Python rules +
Semgrep MVP) and the ES-0..ES-5 sub-iter slate are preserved in
[`extrace-static-stream-handoff.md`](extrace-static-stream-handoff.md).
The `extrace-static` branch + parallel worktree were deleted
`2026-05-28` (the parallel-worktree pattern on a shared-Docker stack
cost more operationally than it saved).
