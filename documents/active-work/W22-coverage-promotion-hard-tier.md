...
### W22-7 — Activation event spec gap (skipped — `[NO-W22-7]` doc-only stamp)

**Status**: **skipped** — doc-only stamp per W22-7 conditional plan.

**Decision rationale**: W20-0 crosswalk surfaced 5 candidate "missing" event names (`onMemento`, `onTerminalQuickFixRequest`, `onChat`, `onAuthenticationProvider`, `onRendererScript`) that the GPT review flagged as NOT present in the official VS Code Activation Events spec. Without a real-world fixture exercising a genuine gap (i.e., a target manifest that uses one of these events and would benefit from Tier-1 implementation), the W22-7 conditional path resolves to skip. If a future analyzer run surfaces a manifest activation event that maps to one of these candidates, file as a fresh W23+ candidate.

**Changes**: none — doc-only stamp recorded in tracker row + this section.

**Test bar delta**: 0 (skip path).
...
**Sub-iter recap** (audit trail):

| Iter | Status | Commits | Goal |
|---|---|---|---|
| W22-0 | closed | `26bb080` + `ff3fbbd` | Doc reconcile + W22 active-work tracker + §20 plan header |
| W22-1 | closed | `906fcd5` + `d018fe1` | ADR 0014 chat + LM tool policy (Option C) + 2 architecture invariants |
| W22-4 | closed | `9a8ad28` + `ea418a6` | ADR 0015 sandbox-evasion defense (Draft Policy, 5-family taxonomy) + 2 architecture invariants |
| W22-5 | closed | `a6dd24b` + `1de616b` | Sandbox-evasion canary fixture (observer-side, 8 cases) + `evasion_signals.py` + Makefile enrolment |
| W22-3 | closed | `cff10d3` + `70dc43a` | Attribution count parity for `kind=process` + `kind=output_channel_appendline` (W17-1 paterni byte-identical) + 8 invariants |
| W22-2 | closed (static cut) | `ffbb743` + `d9e4558` | **Chat hard-tier promotion (HARD TIER CLOSURE STATIC)** — 2 dict flips + 2 scenarios + greenfield extension.js chat participant + LM tool + stimulus_dispatch.js replace + 5 invariants + fixture regen showing `missing_capabilities = []` |
| W22-6 | **deferred to user** | — | Container ratchet-down (read_only + tmpfs + custom seccomp) requires Linux host for live-smoke; user-owned |
| W22-7 | **skipped — [NO-W22-7]** | — | No real activation-event Tier-1 gap surfaced from W20-0 crosswalk |
| W22-N | closed (this commit) | this commit | Close-out hygiene + tracker freeze |

**Deferrals (user-owned, post-session)**:

1. **W22-6 implementation** — Linux required. Resumption checklist documented in W22-6 section above.
2. **W22-2 runtime live-run anchor** — `make sim-target TARGET=ms-python.python` requires docker stack + Linux; expected outcome `coverage_summary.missing_capabilities == []` confirming the static fixture state.
3. **PR `week22 -> main`** — user creates after live-run confirmation. At PR-prep time the canonical preamble parity test fingerprint will need refresh (PR #30 → PR #X, `5dc18aa` → week22 close-out SHA) + `test_readme_phase_pointer.py` will need a `test_readme_phase_pointer_mentions_w22_closeout_merge` gate pinning the new merge fact.

**Final W22 test bar (static; runtime gate user-owned)**:

- `tests/architecture/`: 287 (W21 final) → 292 passed (+5 net: +1 W22-0 README phase-pointer gate + 2 W22-1 ADR invariants + 2 W22-4 ADR invariants).
- `tests/platform/contracts/test_capability_support_invariants.py`: 26 → 31 (+5 W22-2 chat invariants).
- `tests/platform/contracts/test_registry_split_regression.py`: 8 passed (count pin 16 → 18 W22-2).
- `tests/executor/test_playwright_attribution_links.py`: 30 → 38 (+8 W22-3 process + output parity).
- `tests/security/test_sandbox_evasion_canary.py`: 8 (+8 W22-5).
- `make test-security`: 220 → 228 (+8 W22-5 enrolled in target).
- Broader sweep (architecture + contracts + marketplace + attribution): 631 passed, 2 skipped, 4 deselected.

**W22 acceptance bar status**:

| # | Must-pass | Status |
|---|---|---|
| 1 | ADR 0014 Accepted + local-only implemented | ✅ static (live-run pending user) |
| 2 | ADR 0015 Accepted + impl W23+ scope | ✅ |
| 3 | W22-3 attribution parity 8/8 green | ✅ |
| 4 | W22-5 canary fixture green | ✅ |
| 5 | W22-6 container ratchet-down active + smoke pass | ⏭ deferred to user (Linux) |
...
(Self-stamp follow-up for this commit's `(this commit)` placeholders is NOT required — W22-N is a single close-out commit, not a primary+self-stamp pair. Future doc edits that need to backfill the W22-N SHA can do so in the user's PR-prep commit.)
