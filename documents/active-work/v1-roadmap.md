...
| **S5** ext-host ReDoS sweep `[FOLLOWUP exthost-logparse-redos-bounds-sweep]` (done on `week23`) | `executor/host.py:65`; `runtime_capture/extension_host_strace_parse.py`; `health/handshake.py:37`; `runtime_capture/extension_host_log_parse.py` | audited all four regex-bearing ext-host files: the family is line-anchored/linear (strace `^…$`-anchored; harness mask = literal prefix + `\S+`; harness marker per-line single greedy `.*`; log patterns per-line behind an `activ`/`register` pre-filter). The audit found one real edge — `_ACTIVATION_PATTERNS[4]` was the only unanchored greedy-prefix pattern (O(n²) on a colon-less mega-line) — and **fixed** it: bounded the prefix `{1,256}` (linear) + added a 16 KiB per-line cap. 1M-char line minutes→~32 ms; 3 regression tests; 845 executor tests green. Standing "unaudited" flag closed. **Non-blocking.** |
