...
sha256(vsix_bytes).hexdigest()` is computed **once** at the shared ingest
chokepoint `workflows/marketplace/client.persist_and_extract_vsix_bytes` (the
single tail of the online + offline ingest paths). It is surfaced to the
orchestrator and threaded to:
...
| **S1** intake hash chokepoint | `workflows/marketplace/client.py` (`persist_and_extract_vsix_bytes`, compute once at top before the idempotent early-returns; surface via an additive out-param); callers `analysis_execution.py`, `router.py`, `offline.py` | every ingest path (online download + offline intake) yields the same canonical 64-char lowercase `vsix_sha256`; unit test pins online == offline for identical bytes |
...
| **S4** B5 DB persistence | `appcore/storage/model_defs/analysis_job.py` (nullable `vsix_sha256` after `last_heartbeat_at`); `appcore/contracts/schema_defs/analysis_jobs.py` (snapshot/update field); `appcore/storage/crud_ops/analysis_jobs/lifecycle.py` (CRUD facade, `**snapshot.model_dump()`); new alembic revision (additive-nullable, head off `c3f8a1d7e9b2`) | **GATED — migration shown before run.** stale same-version re-publish is not conflated (two byte-different VSIX → two distinct `vsix_sha256` rows); `alembic-upgrade extrace` (5432) then `make check-all` (dev-DB gotcha) |
