...
| Iter | Stable ID(s) | Status |
|---|---|---|
| W20-0 | doc-reconcile — `week20` branch + 9-doc canonical preamble refresh + new active-work tracker for W20 + §18 W20 plan header doc-open + §18-§20 combined header split into §18 W20 active + §19-§20 W21-W22 planning + W20 Pull-Forward Acceptance Bar promotion (this section) + README phase-pointer arch gate transition W19→W20 + new W19 close-out fact gate `test_readme_phase_pointer_mentions_w19_closeout_merge` pinning PR #28 / `week19 -> main` / `c879603` + baseline live-run pending user "go" | **closed `2026-05-26`** via primary `66a8a0b` + self-stamp this commit |
| W20-1 | `[GOAL taxonomy-scm-official-promotion]` (new) — `_OFFICIAL_CAPABILITY_SUPPORT["scm"]: "missing" → "covered"` at [`capabilities.py:88`](../packages/analysis_planner/capabilities.py); schema impact survey (W19-3 paterni) + invariant tests at `tests/platform/contracts/test_coverage_model.py` extend; fixture regen if planner output depends | planned |
| W20-2 | `[GOAL taxonomy-settings-official-promotion]` (new) — `_OFFICIAL_CAPABILITY_SUPPORT["settings"]: "missing" → "covered"` at [`capabilities.py:90`](../packages/analysis_planner/capabilities.py); W20-1 paterni byte-identical | planned |
| W20-3 | `[GOAL coverage-matrix-contract-tests]` (new) — keyset parity (official ↔ heuristic ↔ taxonomy) + Official ⊆ Heuristic subset + `_GLOBAL_CAPABILITY_NOTES` keyset ↔ taxonomy alignment + `CAPABILITY_TAXONOMY` ordering pin + W20-1/W20-2 post-condition regression. Extend `test_coverage_model.py` or new `test_coverage_track_invariants.py` | planned |
| W20-4 | `[DESIGN taxonomy-comments-testing-readiness]` (new) — VS Code Comments API + Test Controller API surface envelope + W21 plumbing şablonu (`testing` W21-1 + `comments` W21-2); doc-only | planned |
| W20-5 | close-out hygiene + 9-doc canonical preamble refresh (Active → Previous flip + final test bar) + §18 W20 self-stamp + W20 tracker freeze + W20 Pull-Forward Acceptance Bar audit-trail close + final live-run on `ms-python.python` + cross-doc parity gate for §18 self-stamp marker (W19-6 paterni) + opportunistic pull of `[FOLLOWUP harness-secret-extra-reactivation-source]` if W20-5 diagnostics show `poll_attempts > 1` + close-out PR `week20 -> main` open **PENDING USER APPROVAL** | planned |
...
Stable IDs `W21-1..W22-7` are reserved and assigned at first pull per W11-W19 precedent. Aşağıdaki satırlar **planned**, henüz açılmadı:
