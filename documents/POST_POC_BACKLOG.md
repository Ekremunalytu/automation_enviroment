...
`Last Updated: 2026-05-13 (W14 sub-iter slate complete on week14 (W14-1..W14-6) + W14-7/W14-8 post-slate hotfixes closed; W14-1 closed via 0c8bd02 (BLOCKER -> HIGH; upstream split -> W15+); W14-2 closed via bde17be; W14-3 closed via 941250d; W14-4 closed via 03b32bc; W14-5 closed via dc79f61+9c095d2+db25d5f; W14-6 closed via 2adad43+b031803+e42a448; W14-7 closed via df925f8+c11ebd8 — container-shipping regression + Python 3.10 UTC compat; W14-8 closed via 5638f82 — preventive Python 3.11+ API gate in container-shipped paths; close-out PR week14 -> main next; off-roadmap hygiene candidates: rule-registry-side-effect-loader, compose-image-mutable-ref-pin, gh-action-trivy-version-pin, test-import-graph-policy-dump-split, report-finalize-top-level-field-sync-drift)`
...
|---|---|---|---|
| W14-1 | `[BUG scenario-dropout-upstream-root-cause]` | **closed (W14 scope)** `2026-05-13` via `0c8bd02` — BLOCKER → HIGH | BLOCKER triage landed on `week14` (deterministic repro matrix at `tests/security/test_scenario_dropout_repro.py` covers 5 known dropout vectors; conservation guard at `scenario_accountant.py:392-438` is the deterministic fix-of-record). Upstream emit-site work tracked under `[FOLLOWUP scenario-accountant-conservation-split]` (W15+) |
...
| W14-7 | `[FOLLOWUP w14-container-shipping-regression]` | **closed** `2026-05-13` (post-slate) | Production smoke retry on `week14` post-`2cbdca0` exposed that W14-6.c (`e42a448`) added `executor/binary_paths.py` and W14-5.3 (`db25d5f`) added `executor/runtime_fingerprint.py` without updating `executor/container/Dockerfile` COPY directives; container `python3 -m executor.flows.playwright.entrypoint --monitor` raised `ModuleNotFoundError: No module named 'executor.binary_paths'` at first import → analysis jobs failed at `run_monitoring` step ("Automation exited before writing the requested report") → UI showed stale "Installing extension in sandbox / 5%" because the report path was never reached. Hotfix (`df925f8`) adds the two missing COPY lines + replaces W14-5.3's `from datetime import UTC` (Python 3.11+) with the established `getattr(_dt, "UTC", _dt.timezone.utc)` compat shim (matches `packages/analysis_engine/runner.py:26`) so the Python 3.10 executor container no longer breaks at import. Regression gate (`c11ebd8`) at `tests/architecture/test_executor_container_shipping.py` parses Dockerfile COPY directives, AST-walks every `executor/flows/**/*.py` and every shipped executor-root file for `from executor.<root>` imports, and asserts every imported root is shipped — pinning the invariant against the next root-module addition. 2 new arch cases (`tests/architecture/` 168 → 170). |
| W14-8 | `[FOLLOWUP w14-container-python-compat-gate]` | **closed** `2026-05-13` (post-slate, preventive) | W14-7.b's container-shipping gate pinned the missing-COPY half of the W14-7 regression but could not catch the incompatible-API half (`from datetime import UTC` is valid 3.11+ syntax; tests run on host Python 3.11+). W14-8 closes that loop with a static AST gate at `tests/architecture/test_executor_container_python_compat.py`: derives the container-shipped Python file list from the same Dockerfile COPY parser as W14-7.b (single source of truth), then AST-walks each file for the most common Python 3.11+ symbols — `from datetime import UTC`, `from typing import (Self/NotRequired/Required/LiteralString/TypeVarTuple/Unpack/ExceptionGroup/BaseExceptionGroup)`, `import tomllib` / `from tomllib import ...`. Per-import allowlist via `# arch-allow: py311-api` pragma (mirrors W14-6's `# arch-allow: bare-binary-path`). 1 new arch case (`tests/architecture/` 170 → 171). |
