...
| W19-2 | `[BUG scenario-unaccounted-dropout-debug-refactor]` — Dropout emit-site fix (Hat-1; W16-1 paterni); ONE-PATH verdict (no mini-ADR); upstream emit-site landed in `executor/flows/playwright/stimulus/passes.py` covered-only branch — new `covered_via_layered_attempts` reason_code; accountant fallback `scenario_accountant.py:392-438` preserved as son-mil koruyucu; +2 W16-1-mirror synthetic unit tests at `tests/security/test_scenario_dropout_repro.py`; W19-1 fixture regenerated (SYNTHESIZED — live re-run verification DEFERRED to W19-6 close-out per `docker-compose build` cycle) | **closed `2026-05-25`** via primary `89b64da` + self-stamp `<this commit>` |
