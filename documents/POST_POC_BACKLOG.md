...
| W19-4 | `[FOLLOWUP harness-verification-debug-events]` (new) — `onDebug*` family nonce confirmation generation in `executor/flows/harness_extension/*`; emit pipeline stamps `confirmation_source="harness_nonce"`; **+ consumer wire** in `executor/flows/playwright/health/reconciliation.py` `_mark_unverified_harness_attempt` path so attempts with `confirmation_source != "none"` skip the `failure_reason_code="harness_verification_unconfirmed"` marker — without this half the W19-3 schema-only landing leaves W19 must-pass #2 `harness_verification_unconfirmed_present` reason drops unmet | pending |
