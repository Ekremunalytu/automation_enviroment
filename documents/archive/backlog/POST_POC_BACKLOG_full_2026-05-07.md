...
`Last Updated: 2026-05-07` (W12 precursor-test gate items landed 2026-05-07; companion W12 tracker scaffold + §11.9.1 plan addendum land on PR #15; 2026-05-07 audit pass surfaced 6 roadmap gaps — 1 W8-6 perimeter escape requires W12 pull-forward)
...
- **`[FOLLOWUP w8-1-vsix-rejection-log-sanitization]`** — VSIX entry
  rejection sites at `workflows/marketplace/client.py:178-181`
  (`reason=path_traversal`) and `workflows/marketplace/client.py:216-219`
  (`reason=symlink_escape`) emit `info.filename` raw via
  `logger.warning("vsix_entry_rejected ... entry=%s", info.filename)`.
  `info.filename` is attacker-controlled (it is literally the rejected
  malicious entry name from the adversarial VSIX); Python's `%s`
  formatter passes newlines and control characters through unchanged,
  so a crafted VSIX can inject arbitrary lines into the operator log
  stream (log forging / operator confusion). Distinct from W8-8 manifest
  log sanitization — that is about logger emits of *manifest field
  values*; this is about logger emits of *archive entry names* on
  the rejection path. AGENTS.md curated rule 9 (treat extension input
  as adversarial) applies. Pickup: introduce a small
  `_safe_log_value()` helper in `workflows/marketplace/client.py` that
  strips control characters + newlines and length-bounds the output;
  apply at both rejection log emits. Test: add a
  `tests/workflows/marketplace/test_vsix_rejection_log_sanitization.py`
  case that builds a synthetic VSIX with a CRLF-bearing entry name
  and asserts the captured log line contains exactly one record (no
  injected line break). **Priority: P2.** Defense-in-depth; not
  eligible for pull-forward. Natural landing: W13
  (`§11.10` regression-lock-in lens). Surfaced 2026-05-07 audit pass
  (Codex cross-check). Lane: `[marketplace-analysis]`.
...
- **`[BUG scenario-dropout-upstream-root-cause]`** —
  `[BUG silent-scenario-dropout-regression]` (closed `2026-05-07`)
  added a last-mile conservation guard in
  `ScenarioAccountant._validate_scenario_conservation` that surfaces
  missing scenarios as `reason_code='unaccounted_dropout'`. The guard
  makes the symptom visible but does **not** fix it — the upstream
  dispatch gap in planner / `stimulus_passes` / harness still drops
  scenarios silently. Live evidence: `2026-05-07` 13:09 ms-python.python
  scan (`output/activation_report_ms-python.python-2026.5.2026042602-2f0f7d59250e.json`) —
  `requested=5` (`coding_session`, `project_exploration`,
  `debug_session`, `terminal_usage`, `refactor_workflow`),
  `scenarios_run=3` (proj/coding/terminal), `failed_scenarios=[]`,
  `skipped_scenarios` carries `debug_session` + `refactor_workflow`
  both with `reason_code='unaccounted_dropout'`. All 5 stimulus
  passes report `status: completed`, so the loss is not a
  stimulus-level failure; the gap sits between trigger-plan request
  and per-scenario dispatch. Pickup: trace the dispatch path that
  feeds `ScenarioAccountant.record_execution_result`
  (`executor/flows/playwright/stimulus_passes.py` plus the planner /
  harness call sites in `executor/flows/playwright/entrypoint_runner.py`);
  add a per-decision drop-reason emit at each branch that elects not
  to dispatch a requested scenario, then pin the upstream emit with
  a regression test that fails today (guard alone is insufficient —
  the upstream layer should record `reason_code` itself, not delegate
  to the last-mile catch). Natural landing: W13 `§11.10`
  (test expansion + observability window) — sits **outside** the
  W12 refactor-only scope (`§11.9`, exit criterion is bitwise-equal
  detection-relevant fields, so behaviour-changing fixes do not fit).
  **Priority: P2.** Surfaced `2026-05-07` post-PR #17 live scan once
  the conservation guard made the dropout visible.
  Roadmap: target landing window **W13 entry (~`2026-06-09`)** if W12
  takes 4-5 weeks, aligned with W12-4 `entrypoint_runner.main`
  dispatch extraction so the upstream `reason_code` emit can be wired
  on top of the cleaned dispatch path (avoids touching the same
  surface twice). Pre-W12 sanity step (~30 min, optional): re-scan
  2-3 non-ms-python extensions (`dbaeumer.vscode-eslint` +
  `esbenp.prettier-vscode` already cached under `output/`, plus one
  fresh) and record whether the dropout is **deterministic**
  (same two scenarios every run) or **stochastic** (drop set varies).
  Promote to **P1** and pull into mid-W12 if either: (a) drop pattern
  varies across extensions (stochastic — coverage uncertainty becomes
  unbounded), or (b) `debug_session` / `refactor_workflow` are
  observed to miss a real threat category in a live target scan.
...
- **`[FOLLOWUP monitor-types-property-recomputation]`** —
  `executor/flows/playwright/monitor_types.py` exposes ~25
  `@property` accessors over the 40-field `ActivationReport`
  dataclass. Several of them call back into the attribution module
  builders on every read; e.g. line 305 `risk_signals` returns
  `_build_risk_signals(self)`, with sibling properties for
  `evidence_events`, `evidence_links`, `risk_summary`. No caching.
  Not currently a defect — no observed hot-path callsite that
  iterates these — but the surface is exactly the kind of thing that
  grows quietly: callers assume O(1) reads, the build funcs grow,
  and one day someone iterates inside a loop. Pickup: defer until
  profiling evidence shows repeated access. If/when needed, either
  precompute at `ScenarioAccountant.finalize_running_scenarios` time
  and store on the dataclass, or memoize with
  `functools.cached_property` (single eval per instance). **Priority:
  P3.** Surfaced 2026-05-07 audit pass.
...
- ~~**`[FOLLOWUP w8-6-output-signals-redaction]`**~~ — closed by
  ...
  Stable ID retained for inbound code/test references; see
  `REFACTOR_STATUS.md` W10 entry.
  **Note (2026-05-07 audit pass):** The close covered the
  `parse_output_signal_events` harness-marker path
  (`output_signals.py:122`) only. The file-backed
  `read_output_channel_logs` path (`output_signals.py:148-217`,
  primary source on VS Code 1.105+) builds `OutputSignalEvent` at
  line 205 without `redact_secrets`. Active W8-6 perimeter escape;
  tracked separately as
  `[FOLLOWUP w8-6-output-signals-file-backed-redaction]` below.
- **`[FOLLOWUP w8-6-output-signals-file-backed-redaction]`** — sibling
  to the closed `[FOLLOWUP w8-6-output-signals-redaction]` parent
  above. The W10-7 close redacted the harness-marker path
  (`output_signals.py:122`) but missed the file-backed
  `read_output_channel_logs` path (`output_signals.py:148-217`).
  VS Code 1.105+ persists Output channel content to
  `<user-data>/logs/<session>/window<N>/exthost/output_logging_<ts>/<idx>-<channel>.log`
  files and the docstring at `output_signals.py:156-161` calls this
  the "primary source" because `console.log` no longer reaches
  `exthost.log`. The reader builds `OutputSignalEvent(text=text)` at
  line 206 with `text = _truncate(line)` (line 205) — no
  `redact_secrets` call. If a target extension writes secrets to its
  Output channel (API keys, DB URLs, OAuth tokens) and VS Code
  persists them to disk, those secrets enter the persisted
  `ActivationReport` unredacted via `OutputSignalEvent.text`.
  Pickup: one-line fix at `output_signals.py:205` —
  `text = redact_secrets(_truncate(line))`. Update the W10-7
  attribution comment at `output_signals.py:117` to acknowledge
  both paths. Test addition: extend
  `tests/platform/security/test_output_signals_redaction.py` (or add
  a sibling `test_output_signals_file_redaction.py`) with fixtures
  that drop AKIA / Bearer / PEM payloads into a synthetic
  `output_logging_*/<idx>-<channel>.log` and assert the resulting
  `OutputSignalEvent.text` contains `[REDACTED-*]` markers, not the
  raw secret. **Priority: P1 — W12 pull-forward candidate.**
  Active W8-6 perimeter regression; not eligible to ride a refactor
  PR. Surfaced 2026-05-07 audit pass (cross-check with external
  reviewer). Lane: `[security-detection]`.
...
- **`[FOLLOWUP arch-gate-network-body-preview-redaction]`** —
  W8-9 P2 close (`executor/flows/playwright/runtime_capture/network.py:156`)
  routes decoded body preview text through `redact_secrets` before
  assigning to `NetworkEvent.{request,response}_body_preview`. That
  is a runtime correctness guarantee, not a structural one — a
  future contributor could add a sibling assignment that bypasses
  the redaction with no AST-level enforcement to catch it. Sibling
  pattern to `tests/architecture/test_marketplace_identity_concat.py`
  (which gate-pins slug helper usage). Proposed gate:
  `tests/architecture/test_network_body_preview_redaction.py` — AST
  parse `runtime_capture/network.py`, find every `Assign` /
  `AugAssign` whose target is `request_body_preview` or
  `response_body_preview` (attribute or keyword arg), and assert the
  source expression contains a `redact_secrets(...)` Call node in
  its dataflow. Pragma escape hatch: `# arch-allow:
  network-body-preview-raw` for any intentional bypass.
  **Priority: P2.** Defense-in-depth structural enforcement.
  Cousin to the proposed
  `[FOLLOWUP arch-gate-bare-binary-pragma-ratchet]` and
  `[FOLLOWUP arch-gate-executor-control-outbound]`.
  Surfaced 2026-05-07 audit pass.
...
- **`[CLEANUP pre-commit-python-version-alignment]`** —
  `.pre-commit-config.yaml:6` declares
  `default_language_version: python: python3.12` and
  `.pre-commit-config.yaml:63` repeats `language_version: python3.12`
  for the mypy hook. The project targets Python 3.11
  (`pyproject.toml` `requires-python = ">=3.11"`,
  `target-version = "py311"`; `docker/api/Dockerfile`
  `FROM python:3.11-slim-bookworm`). Ruff `target-version = py311`
  absorbs most of the discrepancy (Ruff applies py311 rules
  regardless of host interpreter), but mypy run under 3.12 may
  enforce or accept patterns the 3.11 runtime cannot. Low impact in
  practice — the pre-push hook still runs `make check-all` against
  3.11 — but the config drift confuses contributors who notice the
  mismatch. Pickup: change both `python3.12` references to
  `python3.11` (one-line each); confirm pre-commit cache rebuilds
  cleanly. Surfaced 2026-05-07 audit pass.
  Lane: `[docs-maintenance]`.
...
- **`[FOLLOWUP w8-9-network-body-boundary-split-secret-test]`** —
  W8-9 P2 routes body preview through `redact_secrets` (network.py:156)
  but `_HTTP_BODY_PREVIEW_BYTES` truncation (network.py:152) can split
  a secret pattern across the preview cutoff (e.g. an `AKIA…` token
  that begins at byte 95 with cutoff at byte 100 leaves only a
  partial 5-char fragment that the regex misses). Existing tests
  exercise full-token payloads only. Add a parametrised case set
  varying the cutoff position (1..N bytes before / after the secret
  start) for each of the 5 secret classes (aws, bearer, private_key,
  api_key, db_url) — all should still produce `[REDACTED-*]` markers
  or be safely truncated, never a leak. **Priority: P2.** No new
  dependency; pure pytest parametrize. Surfaced 2026-05-07 audit
  pass. Lane: `[security-detection]`.
