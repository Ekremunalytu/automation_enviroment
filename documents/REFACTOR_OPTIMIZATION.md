...
opens at W19-6.** W19-0..W19-2 are closed; W19-3..W19-6 remain
pending. Active tracker:
[`active-work/W19-live-run-root-cause.md`](active-work/W19-live-run-root-cause.md)
...
W19-2 live re-anchor `d5de9ca` now satisfies `unaccounted_dropout == 0`.
Hat-1 is closed + live-verified; Hat-2 remains active in this iter.
...
| W19-6 | reserved (close-out hygiene + 8-doc canonical preamble refresh + §17 W19 self-stamp post-final-bar + W19 tracker freeze + PR `week19 -> main`; W18-4 paterni `3f4f95a`; **+ 3 hygiene items from W19-3-followup-2 audit `2026-05-25`**: (a) field-set parity gate — extend existing `test_executor_dataclass_and_pydantic_contract_share_confirmation_source_field` to a full `set(pydantic_fields) == set(dataclass_fields)` assertion; (b) hotspot LOC ratchet — new `tests/architecture/test_executor_hotspot_loc_ratchet.py` mirroring `test_runner_main_loc_budget.py` pattern, pins 9 modules >500 LOC under `executor/flows/playwright/` at W19-4/W19-5 post-landing LOC + 5% margin; (c) acceptance-bar column — add `Closes which acceptance-bar item?` column to the W19 sub-iter distribution table at §17.3 + active tracker mirror, visual gap = red flag; **+ W20-0 forward reference** `[FOLLOWUP defensive-test-parametrize-helper]` — schema-only field landings share a `_test_schema_only_field(field_name, default, allowed_values)` table-driven helper instead of per-field functions, would have cut W19-3's 22 tests to ~6) | pending |
