...
| W19-6 | reserved (close-out hygiene + 9-doc canonical preamble refresh + §17 W19 self-stamp post-final-bar + W19 tracker freeze + PR `week19 -> main`; W18-4 paterni `3f4f95a`; **+ 3 hygiene items from W19-3-followup-2 audit `2026-05-25`**: (a) field-set parity gate — extend existing `test_executor_dataclass_and_pydantic_contract_share_confirmation_source_field` to a full `set(pydantic_fields) == set(dataclass_fields)` assertion; (b) hotspot LOC ratchet — new `tests/architecture/test_executor_hotspot_loc_ratchet.py` mirroring `test_runner_main_loc_budget.py` pattern, pins **8 modules** >500 LOC under `executor/flows/playwright/` at W19-5 post-landing LOC + 5% margin (W19-4 + W19-5 already landed at `7d44b0e` + `e537ebd` — their LOC growth folds into the baseline; original plan's "9 modules" was a count drift, current set is 8); (c) acceptance-bar column — add `Closes which acceptance-bar item?` column to the W19 sub-iter distribution table at §17.3 + active tracker mirror, visual gap = red flag; **+ W19-X-handoff.md freeze** — add SUPERSEDED banner to the uncommitted handoff doc from the W19-X live close-out session, commit as frozen artifact for the bug-genealogy context; migrate risk-register row #5 (W13-1 design + reload cycle) to POST_POC_BACKLOG.md as `[FOLLOWUP harness-secret-distribution-redesign]`; forward `[FOLLOWUP harness-secret-extra-reactivation-source]` to W18-W22-roadmap.md W20-0; **+ W20-0 forward reference** `[FOLLOWUP defensive-test-parametrize-helper]` — schema-only field landings share a `_test_schema_only_field(field_name, default, allowed_values)` table-driven helper instead of per-field functions, would have cut W19-3's 22 tests to ~6) | pending | expected (`run_quality: low → medium`) + close-out hygiene |
