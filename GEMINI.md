...
**Core Capabilities:**
1.  **Ingest:** Recursively scans extensions to extract metadata from `package.json`.
...
*   **Running API:** `make dev` (local) | `make docker-up` (containerized stack)
*   **Executor Management:**
    *   `make executor-build`: Build the analysis image.
    *   `make executor-up`: Start the virtual display environment.
    *   `make executor-shell`: Access the running sandbox.
    *   `make executor-test`: Verify VS Code and monitoring tools in the container.
